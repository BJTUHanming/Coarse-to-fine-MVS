...
class CostAgg(nn.Module):
    def __init__(self, mode="variance", in_channels=None):
        ...
        if self.mode == "adaptive":
            n_stage = len(in_channels) if in_channels is not None else 3
            self.weight_net = nn.ModuleList([AggWeightNetVolume(in_channels=1) for _ in range(n_stage)])

    def forward(self, features, proj_matrices, depth_values, stage_idx):

        ref_feature, src_features = features[0], features[1:]
        proj_list = torch.unbind(proj_matrices, 1)
        ...
        if self.mode == "variance":
            ...
            for src_fea, src_proj in zip(src_features, src_projs):
                ...
                warped_volume = homo_warping(src_fea, src_proj_new, ref_proj_new, depth_values)  # (B,C,D,H,W)
                volume_sum = volume_sum + warped_volume
                volume_sq_sum = volume_sq_sum + warped_volume ** 2
                del warped_volume

            return volume_sq_sum.div_(num_views).sub_(volume_sum.div_(num_views).pow_(2))
        ...
        for src_fea, src_proj in zip(src_features, src_projs):
            ...
            warped_volume = homo_warping(src_fea, src_proj_new, ref_proj_new, depth_values)      # (B,C,D,H,W)
            similarity = (warped_volume * ref_feature.unsqueeze(2)).mean(1, keepdim=True)        # (B,1,D,H,W)
            del warped_volume

            view_weight = self.weight_net[stage_idx](similarity)                                # (B,1,D,H,W)
            weight = F.relu(view_weight)

            if sim_num is None:
                sim_num = similarity * weight
                w_den = weight
            else:
                sim_num = sim_num + similarity * weight
                w_den = w_den + weight

        return sim_num.div_(w_den.clamp_min(1e-6))                                               # (B,1,D,H,W)
...
class MVSNet(nn.Module):
    def __init__(self, ndepths, depth_interval_ratio, cr_base_chs=None, fea_mode="fpn", agg_mode="variance", depth_mode="regression"):
        ...
        self.cost_regularization = nn.ModuleList([CostRegNet(in_channels=1, base_channels=self.cr_base_chs[i]) for i in range(self.num_stage)])
