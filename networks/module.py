...
class FeatureNet(nn.Module):
    def __init__(self, base_channels, num_stage=3, stride=4, mode="fpn"):
        super().__init__()
        assert mode in ["unet", "fpn"], "mode must be in 'unet' or 'fpn'"
        self.mode, self.num_stage = mode, num_stage
        C = base_channels

        # stem
        self.conv0 = nn.Sequential(
            Conv2d(3, C, 3, 1, padding=1),
            Conv2d(C, C, 3, 1, padding=1),
        )
        self.conv1 = nn.Sequential(
            Conv2d(C, 2*C, 5, stride=2, padding=2),
            Conv2d(2*C, 2*C, 3, 1, padding=1),
            Conv2d(2*C, 2*C, 3, 1, padding=1),
        )
        self.conv2 = nn.Sequential(
            Conv2d(2*C, 4*C, 5, stride=2, padding=2),
            Conv2d(4*C, 4*C, 3, 1, padding=1),
            Conv2d(4*C, 4*C, 3, 1, padding=1),
        )

        if self.mode == "unet":
            self.out1 = nn.Conv2d(4*C, 4*C, 1, bias=False)
            self.out_channels = [4*C]
            if num_stage == 3:
                self.deconv1 = DeConv2dFuse(4*C, 2*C, 3)
                self.deconv2 = DeConv2dFuse(2*C, C, 3)
                self.out2 = nn.Conv2d(2*C, 2*C, 1, bias=False)
                self.out3 = nn.Conv2d(C, C, 1, bias=False)
                self.out_channels += [2*C, C]
            elif num_stage == 2:
                self.deconv1 = DeConv2dFuse(4*C, 2*C, 3)
                self.out2 = nn.Conv2d(2*C, 2*C, 1, bias=False)
                self.out_channels += [2*C]
            ...
            final_ch = 4*C
            if num_stage == 3:
                self.inner1 = nn.Conv2d(2*C, final_ch, 1, bias=True)
                self.inner2 = nn.Conv2d(C, final_ch, 1, bias=True)
                self.out2 = nn.Conv2d(final_ch, 2*C, 3, padding=1, bias=False)
                self.out3 = nn.Conv2d(final_ch, C, 3, padding=1, bias=False)
                self.out_channels += [2*C, C]
            elif num_stage == 2:
                self.inner1 = nn.Conv2d(2*C, final_ch, 1, bias=True)
                self.out2 = nn.Conv2d(final_ch, C, 3, padding=1, bias=False)
                self.out_channels += [C]

            if num_stage == 3:
                self.up3to2 = nn.Conv2d(C, 2*C, 3, stride=2, padding=1, bias=True)
                self.refine2 = nn.Conv2d(2*C, 2*C, 3, padding=1, bias=True)
                self.up2to1 = nn.Conv2d(2*C, 4*C, 3, stride=2, padding=1, bias=True)
                self.refine1 = nn.Conv2d(4*C, 4*C, 3, padding=1, bias=True)
            elif num_stage == 2:
                self.up2to1 = nn.Conv2d(C, 4*C, 3, stride=2, padding=1, bias=True)
                self.refine1 = nn.Conv2d(4*C, 4*C, 3, padding=1, bias=True)


            self.se_top = SEBlock(4*C)

    def forward(self, x):
        c0 = self.conv0(x)   # C, H, W
        c1 = self.conv1(c0)  # 2C, H/2, W/2
        c2 = self.conv2(c1)  # 4C, H/4, W/4

        out = {}
        if self.mode == "unet":
            o1 = self.out1(c2); out["stage1"] = o1
            if self.num_stage == 3:
                i1 = self.deconv1(c1, o1); out["stage2"] = self.out2(i1)
                i2 = self.deconv2(c0, i1); out["stage3"] = self.out3(i2)
        ...
        F1 = self.out1(c2)
        if self.num_stage == 3:
            i = F.interpolate(c2, scale_factor=2, mode="nearest") + self.inner1(c1)
            F2 = self.out2(i)             # -> 2C, H/2, W/2
            i = F.interpolate(i, scale_factor=2, mode="nearest") + self.inner2(c0)
            F3 = self.out3(i)             # -> C, H, W

            # bottom-up enhancement
            S3 = F3
            S2 = self.up3to2(S3) + F2
            S2_hat = self.refine2(S2)
            S1 = self.up2to1(S2) + F1
            S1_hat = self.refine1(S1)
            S1_hat = self.se_top(S1_hat)

            out["stage1"], out["stage2"], out["stage3"] = S1_hat, S2_hat, S3
            return out
...
class AggWeightNetVolume(nn.Module):
    def __init__(self, in_channels=1, mid_channels=12):
        super(AggWeightNetVolume, self).__init__()
        self.w_net = nn.Sequential(
            Conv3d(in_channels, mid_channels, kernel_size=3, stride=1, padding=1),
            Conv3d(mid_channels, mid_channels, kernel_size=3, stride=1, padding=1),
            Conv3d(mid_channels, 1, kernel_size=1, stride=1, padding=0)
        )
